...
public final class PncUtils {
    ...
    public static long convertPncIdtoKojiId(String id) {
        LOGGER.debug("convertPncIdtoKojiId id: {}", id);

        boolean needDecode = true;
        long result = 0L;

        if (NUMBER_PATTERN.matcher(id).matches()) {
            LOGGER.debug("id is numeric");

            try {
                long longId = Long.parseLong(id);

                if (longId < MAX_OLD_ID) {
                    result = longId;
                    needDecode = false;
                }
            } catch (NumberFormatException e) {
                LOGGER.debug("id is not numeric");
            }
        }

        if (needDecode) {
            result = Numbers.base32ToDecimal(id);
        }

        LOGGER.debug("Coverted PNC ID to Koji ID: {} --> {}", id, result);
